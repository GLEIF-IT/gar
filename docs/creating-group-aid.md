...
the steps definied in the rest of this document to ensure proper compliance with the identity authentication requirements of the vLEI EGF.
...
To challege all the other participants in your group, you must generate a random challenge phrase using the 
...
is wating for a valid response (the 12-word phase will be different every time).
...
command.  When using autocomplete if there are more than on completion when you hit TAB the system will beep.  At this point
if you hit TAB again, all possible completions will display below the current command.  Typing enough to uniquly identify
...
remember the Alais you provided, you can hit the TAB key twice after add a space after `add local` to show your the aliases
for your local AIDs.  
...
This shows the full configurationt that will be passed to the Inception command to create the group multisig AID.  You need
...
new group multisig AID and for the filename created in the last step.  Remeber that the filenames are relative to the
...
the external and internal AIDs repectively.

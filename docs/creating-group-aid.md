...
chat and assign them a local alias that you will use to refer to them in your local GAR environment.  
...
still on the same live video session you can move on to the next step to authenticate these contacts.
...
phrase that the other participant must sign and return.  This ensures that the other participant in the group is the
sole controller of the private key of the AID resolved from that participant during the OOBI exchange process.

To challenge all the other participants in your group you must generate a separate random challenge phrase for each participant using the 
following command and paste it in a PRIVATE chat specific to each participant.  The script will prompt you for the Alias
...
protocol message.  Once you have received a valid signed challenge response containing the 12-word phrase from the other participant
then you can run the respond script and input the 12-word phrase to complete the process.
...
### Challenge and Response Process Detailed Steps

First, generate a challenge with the following script.
...
is waiting for a valid response (the 12-word phrase will be different every time).
...
During this process you will also be challenged with unique 12-word challenge phrase from the other participant that you must 
sign and return to the originator of the challenge.  When you receive a 12-word challenge from another participant you must 
sign it and return that signed response to the other participant using the following command:
...
contacts using the utility script `./scripts/contacts.sh` which will display the contact, their AID and the authenticated
state of that contact.  

## Configure Group Multisig
As mentioned earlier in the document, once the OOBI and challenge response phrase has been completed for every participant
pair in the group (meaning everyone has authenticated contacts for everyone else), a Lead must be selected from the group to
perform the step of configuring the multisig group by generating and sending the inception event to everyone else.  The KERI
...
### GLEIF Root AID as Delegator
The GAR software was provided with configuration information that allowed the system to resolve and authenticate the GLEIF 
RoOT AID.  An OOBI was configured to load the key event log along with 5 well known OOBIs provided for authentication of 
the AID as the Root AID.  The AID for GLEIF Root was added as a contact to your database name "GLEIF Root" when it was first
initialized and that contact was authenticated by resolving the well knowns in the database.  The vLEI EGF mandates that
the GLEIF External and Internal AIDs must be created as delegated identifiers and use the GLEIF Root AID as the delegator.

The KIMS shell provides a command for declaring a delegator as part of a group multisig configuration.  The first thing the 
Lead GAR should do after launching the shell is to set the delegator as GLEIF Root.  The following command is how to accomplish
this:

```bash
(kims) delegator GLEIF Root
delegator set to GLEIF Root (EBLq6yb5aAFgk69INxz-yNp36SXkCgk4hv0wu8NlGr_0)
```
...
this now by typing `dele` at the `(kims)` prompt and hitting the TAB key.  The full `delegator` command should complete.
If you now add a space and the type `GLE` and hit the TAB key again you should see `GLEIF Root` complete to form the complete
command.  When using autocomplete if there is more than one completion when you hit TAB the system will beep.  At this point
if you hit TAB again, all possible completions will display below the current command.  Typing enough to uniquely identify
one of the completions and hitting TAB will fill in the rest.

### Adding Local AID
Since the Lead GAR is the one creating the multisig configuration, you should add your local AID as the first participant
...
The vLEI EGF also mandates that the GLEIF AIDs all use fractionally weighted thresholds.  This means that for each participant
...
provided the threshold for each member in your GLEIF Group Multisig AID prior to beginning this procedure.
...
information loaded when you created your database environment included OOBIs for the witnesses in the GLEIF provided vLEI
Witness Network.  Those witnesses were loaded and assigned aliases to uniquely identify each witness by region and cloud
provider.  The configuration for the GLEIF AIDs is required to contain a diverse set of witnesses across region and cloud
...
```bash
(kims) add witness gleif-aws-us-east-1
gleif-aws-us-east-1 AID BBilc4-L3tFUnfM_wJr4S4OJanAv_VmF_dJNN6vkf2Ha added to witnesses
(kims)
```
...
(kims) show
{
  "delpre": "EGQeIsQd_vGIZ4zlHuMMqJWMjwKqAUOV5v-yCqcMWqAN",
  "aids": [
...
Assuming you wish to save this file with the name "gleif-external-aid-inception.json" you would use the `save` command in
KIMS as follows:

```bash
(kims) save /data/gleif-external-aid-inception.json
```
...
As the Lead GAR, now that you have created the inception configuration file for the group multisig AID you need to use the
KERI system to create the inception event (thus creating the AID) and submit it to the other participants of the group. 
You will use the `multisig-incept.sh` script to initiate the process.  The script will prompt you for the local alias of the
...
Assuming the configuration file created in the last step and an Alias of "GLEIF External AID", the command initiation will 
look like the following:

```bash
$ ./scripts/multisig-incept.sh
Enter an Alias for the Group Multisig AID: GLEIF External AID
Enter the filename of the inception configuration file: /data/gleif-external-aid-inception.json
...
multisig AID (described in the next section for all other participants).  Once you receive enough signatures to finish the 
event your script will submit the event and all signatures to your delegator (GLEIF Root AID) for approval.  Once you receive
the approval from the GLEIF Root AID, your script will submit the event and all signatures to the witness to receive 
receipts for the events and then propagate those receipts to all other witnesses.  Once that step is complete, your command 
should look like the following as it updates you with the progress along the way:

```bash
$ ./scripts/multisig-incept.sh
Enter an Alias for the Group Multisig AID: GLEIF Root AID
Enter the filename of the inception configuration file: /data/gleif-aid-incept.json
Group identifier inception initialized for EDgkEWEfAnHTNmbHj4oJvOfYyFJC93lJ19WB2GOwFaNI
Sending multisig event to 6 other participants
Waiting for other signatures for 0...
...
## Join Multisig Group
If you were not elected to be the Lead for the creation of your GLEIF Group Multisig AID you will wait until the lead has
...
| Name                |                           Value                                |
+---------------------+----------------------------------------------------------------+
| Delegator           | GLEIF Root AID (EK7j7BobKFpH9yki4kwyIUuT-yQANSntS8u1hlhFYFcg)) |
...
Enter alias for new AID: GLEIF External AID
...
AID that you will be joining.  It is recommended that the aliases "GLEIF External AID" and "GLEIF Internal AID" be used for
...
[create their AIDs through delegation](../external/docs/approving-qvi-inception.md), 
[approve rotation requests from QVIs](../external/docs/approving-qvi-rotation.md) and the 
[issuance](../external/docs/issuing-qvi-credentials.md) and [revocation](../external/docs/revoking-qvi-credentials.md)
...
the holder of the Legal Entity vLEI Credential, Internal GARs must [accept the credential issuance](../internal/docs/accept-legal-entity-credential.md)
of the Legal Entity vLEI credential from a QVI.  It is also the responsibility of the GLEIF Internal AID to 
[issue role authorization credentials](../internal/docs/authorize-role-credentials.md) to QVIs to authorize the QVI to issue

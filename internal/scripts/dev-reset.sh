...
##################################################################
##                                                              ##
##     Dev Reset Script - Teardown and recreate Internal GAR    ##
...
BACKUP_PATH="$BACKUP_DIR/int-gar-$TIMESTAMP"
...
if [ -n "${passcode_item}" ]; then
    echo "Deleting int-gar-passcode from Keychain"
...
if [ -n "${salt_item}" ]; then
    echo "Deleting int-gar-salt from Keychain"
...
kli init --name "${INT_GAR_NAME}" --salt "${salt}" --passcode "${passcode}" --config-dir /scripts --config-file test-int-gar-config.json

# Create local AID with test pool witnesses
kli incept --name "${INT_GAR_NAME}" --alias "${INT_GAR_ALIAS}" --passcode "${passcode}" --file /scripts/test-incept-pool-1.json
...
echo "New Internal GAR AID created with test pool"

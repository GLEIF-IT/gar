...
cp "${msgfile}" "${INT_GAR_DATA_DIR}/.probe-message"
trap 'rm -f "${INT_GAR_DATA_DIR}/.probe-message"' EXIT
...
docker run -it --rm \
  ...
  "${KERI_IMAGE}" \
  /scripts/exn_probe.py --name "${INT_GAR_NAME}" --passcode "${passcode}" --file /data/.probe-message "$@" \
  | tr -d '\r'

...
function create_aid() {
    ...
    print_green "Created AID: ${NAME} with prefix: ${PREFIX}"
    print_bg_blue "\tPrefix: ${PREFIX}"
...
function create_gida_multisig() {
    if test -f $HOME/.keri/full-chain-gida-ms; then
    print_dark_gray "[Internal] GIDA Multisig AID ${GIDA_MS} | ${GIDA_PRE} already exists"
    return
    fi
